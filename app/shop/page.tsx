              ...
              {filteredProducts.map((product, index) => (
                <div key={product.id} className="relative flex flex-col group">
                  <Link href={`/product/${product.name.toLowerCase().replace(/\s+/g, '-')}`} className="block relative aspect-square bg-gray-100 mb-3 overflow-hidden rounded-md">
                    {(product.customBadge || product.isBestSeller || (product.stock !== undefined && product.stock <= 0)) && (
                      <div className="absolute top-3 -left-1 z-20 flex flex-col items-start gap-2">
                        {product.stock !== undefined && product.stock <= 0 ? (
                          <>
                            <div 
                              className="bg-red-600 text-white text-[10px] font-semibold py-0.5 pl-2.5 pr-4 shadow-sm uppercase tracking-wider" 
                              style={{ clipPath: 'polygon(0 0, 100% 0, 85% 50%, 100% 100%, 0 100%)' }}
                            >
                              Out of Stock
                            </div>
                            <div 
                              className="w-1.5 h-1.5 bg-red-800 -mt-2" 
                          ...
                          <>
                            ...
                            <div 
                              className="w-1.5 h-1.5 bg-[#07474B] -mt-2" 
                              style={{ clipPath: 'polygon(100% 0, 100% 100%, 0 0)' }} 
                            />
                          </>
                        )}
