...
export default function SimilarProducts({ currentProductId }: SimilarProductsProps) {
  ...
  useEffect(() => {
    try {
      ...
      if (Array.isArray(stored)) {
        setRecentlyViewedIds(stored.filter((id: string) => id !== String(currentProductId)));
        
        // Add current product to the front of recently viewed history
        const updated = [String(currentProductId), ...stored.filter((id: string) => id !== String(currentProductId))].slice(0, 10);
        localStorage.setItem('recentlyViewed', JSON.stringify(updated));
      }
    } catch (e) {
      console.error('Error with recentlyViewed localStorage', e);
    }
  }, [currentProductId]);
  ...
  // Filter out the current product, match category, and grab up to 4 similar items
  const displayProducts = products
    .filter(p => String(p.id) !== String(currentProductId) && (!currentProduct || p.categoryId === currentProduct.categoryId))
    .slice(0, 4);
  ...
  return (
    <div id="similar-products" className="max-w-7xl mx-auto px-4 md:px-8 mt-24 border-t border-gray-200 pt-16">
      ...
      {displayProducts.length > 0 && (
        <div className="mb-16">
          <h2 className="text-xl text-black mb-8">Related Products</h2>
      ...
      {recentlyViewedProducts.length > 0 && (
        <div className={displayProducts.length > 0 ? "pt-12 border-t border-gray-100" : ""}>
