...
// Move description robustly using regex
// The policy block starts at: {/* Policy Dropdowns
// The description block starts at: {/* Product Description Expandable */}
// The description block ends at the 3rd closing div after it, just before </div> </div> </div> for the layout.

const fullRegex = /(\{?\/\* Policy Dropdowns[\s\S]*?\}?\s*<\/div>)\s*(\{?\/\* Product Description Expandable \*\/\}[\s\S]*?<\/div>\s*<\/div>\s*<\/div>)/;
const match2 = content.match(fullRegex);
if (match2) {
  const policyBlock = match2[1];
  const descBlock = match2[2];
  content = content.replace(fullRegex, descBlock + '\n\n' + policyBlock);
}
